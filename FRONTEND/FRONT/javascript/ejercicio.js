...
formulario.addEventListener("submit", (element) =>{ 
    ...
    fetch(url,{
        ...
        respuesta.json().then(data => {
            ...
            //var currentId = idCarrier(data.user.insertId)
        })
...
// function idCarrier(id) {
//     let idCurrentUser = id;
...
if(id_URL[0].indexOf("buscarUsuario.html") >= 0 && id_URL[1] != undefined)
    ...
    formulario.addEventListener("submit", (element) =>{ 
        element.preventDefault();
        let url = 'http://localhost:3050/usuarios/actualizarUsuario';
        fetch(url,{
            method: 'POST', // *GET, POST, PUT, DELETE, etc
            mode: 'cors',
            headers: {
